...
import { Calendar, Callout, TextField, ICalendarProps, Stack, values, } from "@fluentui/react";
...
export function DateTimePicker({
    ...
    useEffect(() => {
        onSelectDateTime && onSelectDateTime(fullDate);
    }, [fullDate]);
    ...
    const onSelectTime = ([hours, minutes]: [number, number]) => {
        const newDate = fullDate ? new Date(fullDate.getFullYear(), fullDate?.getMonth(), fullDate.getDate(), hours, minutes)
    ...
    return (
        ...
        <>
            <TextField
                ...
                id={textFieldId}
            />
            {isCallOutVisible && (<Callout onDismiss={onDismiss} target={`#${textFieldId}`}>
                <Stack horizontal={true}>
                    <Calendar
                        {...calendarProps} minDate={minDate} maxDate={maxDate} onSelectDate={onSelectDate} isMonthPickerVisible={false} />
                    <TimePicker
