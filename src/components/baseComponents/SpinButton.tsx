...
import { Stack, Button, TextField, Callout, List } from "@fluentui/react";
...
export function SpinButton<T extends number | string>({
    ...
    useEffect(() => {
        ...
        onSelectItem && onSelectItem(selectItem);
    }, [index]);
    ...
    const onRenderCell = (item: T | undefined, ind: number | undefined) => {
        ...
        return (<div
            style={{ cursor: "pointer", width: CallOutItemWidth, height: CallOutItemHeight, textAlign: "center" }}
            onMouseDown={onClick}
            onKeyDown={onClick}
        >{item?.toString() ?? ""}</div>)
    }
    ...
    return (
        <Stack verticalAlign="center" {...props}>
            ...
            <TextField
                ...
                onBlur={onBlur}
            />
            {isCallOutVisible && (<Callout target={`#${textFieldId}`} >
                <List
