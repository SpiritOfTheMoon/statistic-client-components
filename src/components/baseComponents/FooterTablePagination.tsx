...
import {
  Stack, DefaultButton, TextField, Label,
} from '@fluentui/react';
...
export function FooterTablePagination({
  defaultCurrent, totalCount, onChange, pageSize,
}: FooterTablePaginationProps): JSX.Element {
  const totalPages = (totalCount % pageSize) > 0
    ? Math.round(totalCount / (pageSize + 1))
    : Math.round(totalCount / pageSize);
  const [current, setCurrent] = useState(defaultCurrent);
  const [disabledForward, setDisabledForward] = useState(!(totalPages >= current));
  const [disabledBack, setDisabledBack] = useState(!(current > 1));
  ...
  const onKeyPress = (e: React.KeyboardEvent<HTMLElement>) => {
    if (e.key === 'Enter') {
      ...
      } else setCurrent(valueTextField);
      if (valueTextField < 1) {
        setValueTextField(1);
        setCurrent(1);
      }
